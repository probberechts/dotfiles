...
function! s:FzfRelevant(...) abort
  let l:path = dkogit#GetRoot(expand('%:p:h'))
  if empty(l:path)
  ...
  let l:prompt = l:base . '::' . dkogit#GetBranch(l:path)

  let l:filepaths = dkogit#GetRelevant(l:path, a:000)
  let l:source =  filter(
        \   l:filepaths,
...
if dkogrepper#Get().command ==# 'rg'
  command! -bang -nargs=* FZFGrepper
        ...
        \   <bang>0
        \ )
elseif dkogrepper#Get().command ==# 'ag'
