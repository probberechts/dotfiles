...
function! dkoproject#MarkBuffer(...) abort
  let l:bufnr = s:BufnrFromArgs(a:000)
  call dkoproject#SetRoot('')      " clear
  call dkoproject#GetRoot(l:bufnr) " force reset
  call setbufvar(l:bufnr, 'dko_branch', dkogit#GetBranch(expand('%:p:h')))
endfunction
...
function! dkoproject#GetRoot(...) abort
  ...
  if empty(getbufvar(l:bufnr, 'dko_project_root', ''))
    ...
    let l:root = dkoproject#GetRootByFileMarker(s:markers)

    " Try git root
    let l:path = dkoproject#GetFilePath(get(a:, 1, ''))
    let l:gitroot = dkoproject#GetGitRootByFile(l:path)
    if !empty(l:gitroot)
...
function! dkoproject#SetRoot(root) abort
  let b:dko_project_root = a:root
endfunction

" @return {Boolean} if gitroot and project root differ
function! dkoproject#IsMonorepo() abort
  if empty(dkoproject#GetRoot()) | return 0 | endif
...
function! dkoproject#GetGitRootByFile(path) abort
  if !exists('b:dko_project_gitroot')
    let b:dko_project_gitroot = dkogit#GetRoot(a:path)
  endif
...
" @cached
" @return {String[]}
function! dkoproject#Type() abort
...
function! dkoproject#GetFile(filename) abort
  ...
  let l:project_root = dkoproject#IsMonorepo()
        \ ? get(b:, 'dko_project_gitroot', dkoproject#GetRoot())
        \ : dkoproject#GetRoot()
...
function! dkoproject#GetCandidate(candidates) abort
  for l:candidate in a:candidates
    let l:attempt = dkoproject#GetFile(l:candidate)
    if !empty(l:attempt)
      return l:attempt
    endif
  endfor
...
function! dkoproject#GetPrettierrc() abort
  if !exists('b:dko_project_prettierrc')
    let b:dko_project_prettierrc =
          \ dkoproject#GetCandidate(s:prettierrc_candidates)
