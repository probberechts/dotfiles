...
" @param {string} ft
" @param {string} maker
function! dkoneomake#AddMaker(ft, maker) abort
...
" @return {dict}
function! dkoneomake#NpxMaker(settings, ...) abort
...
function! dkoneomake#IsMakerExecutable(name, ...) abort
  let l:ft = neomake#utils#get_ft_confname(get(a:, 1, &filetype))
  if empty(l:ft) | return 0 | endif

  " dkoneomake#LocalMaker successfully determined a project-local
...
function! dkoneomake#LocalMaker(settings) abort
  ...
  let l:is_executable = !empty(l:exe)
        \ || dkoneomake#IsMakerExecutable(a:settings['maker'], a:settings['ft'])
